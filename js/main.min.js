...
window.addEventListener('DOMContentLoaded', function () {
  ...
  function renderIframe(slider) {
    var sliderElement = slider.wrapperEl;
    var iframes = sliderElement.querySelectorAll('iframe');
    ...
    slides.forEach(function (item, index) {
      if (index === slider.activeIndex && item.closest('iframe') && item.closest('.tour-card__audio')) {
        var audioBlock = item.querySelector('.tour-card__audio');
      ...
      if (isActiveAudio && !item.closest('iframe') && item.closest('.tour-card__audio')) {
        var _audioBlock = item.querySelector('.tour-card__audio');
        _audioBlock.innerHTML = '';
        audioFrame.src = '';
        _audioBlock.appendChild(audioFrame);
      }
      if (item.closest('.swiper-slide-active') && item.closest('iframe') && item.closest('.tour-card__video')) {
        var videoBlock = item.querySelector('.tour-card__video');
        videoFrame.src = videoSrc;
        videoBlock.innerHTML = '';
        videoBlock.appendChild(videoFrame);
      }
      if (isActiveVideo && !item.closest('iframe') && item.closest('.tour-card__video')) {
        var _videoBlock = item.querySelector('.tour-card__video');
        _videoBlock.innerHTML = '';
        _videoBlock.appendChild(videoFrame);
      }
    });
  }
  ...
  mapViewer();
  var videoSrc = 'https://www.youtube.com/embed/9TZXsZItgdw?&autoplay=1';
  var audioSrc = 'https://music.yandex.ru/iframe/#track/112912322/25474374';
  var videoFrame;
  var audioFrame;
  var isActiveAudio = false;
  ...
  window.addEventListener('load', function () {
    ...
    var playVideo = function playVideo() {
      var videoPlayer = document.querySelectorAll('.tour-card__video');
      if (videoPlayer.length) {
        videoPlayer.forEach(function (item) {
          var playButton = item.querySelector('.tour-card__play-button');
          var previewer = item.querySelector('picture');
